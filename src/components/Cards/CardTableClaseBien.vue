    ...
    <div class="rounded-t mb-0 px-4 py-3 border-0">
      <div class="flex flex-wrap p-4 items-center">
        <div class="relative flex w-1/3 px-4 py-2 flex-grow flex-1">
          <i class="fas fa-boxes pt-2 pr-2"></i>
      ...
      <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
        <div>
          <p class="text-sm text-gray-700">
            Mostrando del
            {{ ' ' }}
            <span class="font-medium">{{ claseBienStore.clasesBienes.meta !== undefined ?
                claseBienStore.clasesBienes.meta.from : 0
            }}</span>
            {{ ' ' }}
            al
            {{ ' ' }}
            <span class="font-medium">{{ claseBienStore.clasesBienes.meta !== undefined ?
                claseBienStore.clasesBienes.meta.to : 0
            }}</span>
            {{ ' ' }}
            de
            {{ ' ' }}
            <span class="font-medium">{{ claseBienStore.clasesBienes.meta !== undefined ?
                claseBienStore.clasesBienes.meta.total : 0
            }}</span>
            {{ ' ' }}
            registros
          </p>
        ...
        <div>
          <paginate :page-count="lastPage" :click-handler="page => claseBienStore.getClasesBienes(page)"
