  ...
  <div :class="[color === 'light' ? 'bg-white' : 'bg-emerald-900 text-white']"
    ...
    <div class="rounded-t mb-0 px-4 py-3 border-0">
      ...
      <div class="flex flex-wrap items-center lg:flex-grow  ">
        ...
        <InputFilter v-if="filtroListo" v-model:modelValue="almacenBuscadoID" :cantidad-filtros="cantidadFiltros"
                     :data-options="almacenStore.allAlmacenes" :default-label="'Área'"/>
      </div>
    </div>
      ...
      <div class="overflow-x-auto relative shadow-md mx-0 rounded-none sm:mx-8 sm:mb-6 sm:rounded-lg">
        <table class="w-full text-sm text-left text-gray-800 dark:text-gray-400">
          ...
          <tr v-for="(bien,i) in bienesStore.bienes.data" :key="bien.id" :class="[color === 'light' ? 'hover:bg-lightBlue-100' : 'hover:bg-lightBlue-100 hover:text-black', i % 2 === 0 ? 'bg-white' : 'bg-warmGray-50']"
            ...
            <td
                class="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
              <i :class="[bien.is_active ? 'text-green-600' : 'text-red-500']"
...
const emit = defineEmits(['sendBatchId'])
...
const generarReporte = async () => {
  ...
  const data = await bienesStore.generarReporte(almacenBuscadoID.value);

  batchId.value = data.data.data.batch.id
  emit('sendBatchId', batchId.value)
